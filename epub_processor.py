...
from html_cleaner import (
    ...
    add_chapter_page_breaks, strip_unnecessary_attributes, process_xhtml_single_pass
)
from text_cleaner import clean_text_content, TextCleanOptions, TextCleanReport
from epub_packager import (
    extract_epub, extract_selective_from_epub, package_epub, remove_os_artifacts, has_drm, find_opf_path
)
    ...
    try:
        ...
        for i, img_path in enumerate(image_files):
            ...
            for j, result in enumerate(results):
                if result.was_converted:
                    ...
                    # Track high-level format category changes instead of raw pixel dimensions
                    if 'clamped' in result.details or 'resized' in result.details:
                        detail_key = 'clamped & resized'
                    elif 'grayscale' in result.details or 'dithered' in result.details:
                        detail_key = '4-level grayscale'
                    else:
                        detail_key = 'baseline JPEG'
        ...
        for xhtml_path in content_files['xhtml']:
            if os.path.exists(xhtml_path):
                ...
                # Collect used CSS selectors while in memory if remove_unused_css is enabled
                if options.remove_unused_css:
                    classes, ids, elements = collect_used_selectors(cleaned_bytes)
...
def extract_epub_metadata(epub_path: str) -> dict:
    ...
    try:
        # Fast selective extraction: only unpack metadata XMLs and images
        extract_selective_from_epub(epub_path, work_dir)

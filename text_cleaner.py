...
RE_DOTS_OVER_3 = re.compile(r'\.{4,}')
RE_PUNCT_CAP = re.compile(r'([.!?])([A-Z])')
RE_MULTI_COMMA = re.compile(r',{2,}')
RE_EXCL_QUEST = re.compile(r'([!?]){4,}')
...
def _fix_whitespace(text: str) -> tuple[str, int]:
    ...
    # Fast-path check: skip regex if text contains no multiple spaces/tabs
    if '  ' in text or '\t' in text:
        result, n = RE_MULTI_SPACE.subn(' ', text)
        count += n

    # Space before punctuation (but not ellipsis)
    if any(p in result for p in (' .', ' ,', ' ;', ' :', ' !', ' ?')):
        result, n = RE_SPACE_PUNCT.subn(r'\1', result)
        count += n
...
def _fix_punctuation(text: str) -> tuple[str, int]:
    ...
    # 4+ dots → ellipsis
    if '....' in text:
        text, n = RE_DOTS_OVER_3.subn('...', text)
        count += n

    # Missing space after sentence-ending punctuation before uppercase letter
    text, n = RE_PUNCT_CAP.subn(r'\1 \2', text)
    count += n

    # Multiple consecutive commas
    if ',,' in text:
        text, n = RE_MULTI_COMMA.subn(',', text)
        count += n

    # Multiple consecutive exclamation/question marks (more than 3)
    text, n = RE_EXCL_QUEST.subn(r'\1\1\1', text)
    count += n

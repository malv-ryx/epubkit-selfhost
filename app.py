...
async def cleanup_old_files():
    """Remove temp files older than 1 hour and clean task dictionary."""
    while True:
        ...
        # Purge task metadata dictionary memory leak
        expired_tasks = [
            tid for tid, tinfo in list(tasks.items())
            if tinfo.get("created_at", 0) < cutoff
        ]
...
async def upload_files(files: list[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        # Save uploaded file via chunked streaming to prevent RAM spike
    ...
    async def generate():
        ...
        final = {
            ...
            "report": {
                ...
                "svg_images_unwrapped": report.svg_images_unwrapped,
                "custom_patterns_removed": report.custom_patterns_removed,
                "toc_status": report.toc_status,
                "metadata_items_stripped": report.metadata_items_stripped,
                "whitespace_cleaned": report.whitespace_cleaned,
                "os_artifacts_removed": report.os_artifacts_removed,

...
def extract_selective_from_epub(epub_path: str, dest_dir: str, allowed_extensions: set[str] = None) -> None:
    """
    Selectively extract files matching allowed extensions (e.g. .xml, .opf, container.xml, image extensions).
    Prevents extracting huge text files/fonts when only metadata/cover is needed.
    """
    dest = os.path.abspath(dest_dir)
    if allowed_extensions is None:
        allowed_extensions = {'.opf', '.xml', '.png', '.jpg', '.jpeg', '.gif', '.webp'}
    ...
    with zipfile.ZipFile(epub_path, 'r') as zf:
        for entry in zf.namelist():
            ...
            if ext in allowed_extensions or name in ('container.xml', 'mimetype'):

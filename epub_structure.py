...
def _build_basename_rename_map(rename_map: dict) -> dict[str, str]:
    """Build a mapping of old filename basenames to new filename basenames."""
    return {Path(old).name: Path(new).name for old, new in rename_map.items()}


def _update_css_urls(css_text: str, rename_map: dict) -> str:
    """Replace url() references in CSS text."""
    if not rename_map or 'url(' not in css_text:
        return css_text

    basename_map = _build_basename_rename_map(rename_map)

    def replacer(match):
        url = match.group(1).strip("'\"")
        decoded = unquote(url)
        decoded_name = Path(decoded).name
        if decoded_name in basename_map:
            new_name = basename_map[decoded_name]
            return f'url({decoded.replace(decoded_name, new_name)})'
        return match.group(0)

    return RE_CSS_URL.sub(replacer, css_text)


def _resolve_reference(ref: str, rename_map: dict) -> str:
    """Try to match a reference against the rename map."""
    if not ref or not rename_map:
        return ref
    decoded = unquote(ref)
    ref_name = Path(decoded).name
    basename_map = _build_basename_rename_map(rename_map)

    if ref_name in basename_map:
        new_name = basename_map[ref_name]
        return decoded.replace(ref_name, new_name)

...
def process_xhtml_single_pass(
    ...
    def _process_text(text: str) -> str:
        ...
        if text_options.fix_ocr:
            text, lig_n, quote_n = _fix_ocr_artifacts(text, text_options.normalize_quotes)
            text_report.ocr_ligatures_fixed += lig_n
            text_report.smart_quotes_normalized += quote_n
        if text_options.fix_encoding:
            text, n = _fix_mojibake(text)
            text_report.encoding_issues_fixed += n
        if text_options.fix_punctuation:
            text, n = _fix_punctuation(text)
            text_report.punctuation_fixed += n
        if text_options.normalize_unicode:
            normalized = unicodedata.normalize('NFC', text)
            if normalized != text:
                text_report.unicode_normalized += 1
            text = normalized
        return text
    ...
    for el in tree.iter():
        ...
        # 2c. Text cleanup
        if text_options and tag_local not in SKIP_TAGS:
            if el.text:
                el.text = _process_text(el.text)
            if el.tail:
                el.tail = _process_text(el.tail)

    # Handle tail of empty element streak
    if normalize_ws and len(empty_streak) > 1:
        for empty_el in empty_streak[1:]:
            parent = empty_el.getparent()
            if parent is not None:
                if empty_el.tail:
    ...
    if add_breaks:
        ...
        if head is None:
            head = tree.find('.//head')
        if head is not None:
            existing_styles = head.findall('.//{http://www.w3.org/1999/xhtml}style') or head.findall('.//style')
            has_page_break = any(s.text and 'page-break-before' in s.text for s in existing_styles)
            if not has_page_break:
                ns = tree.tag.split('}')[0] + '}' if '}' in tree.tag else ''
                style_el = etree.SubElement(head, f'{ns}style', type='text/css')
                style_el.text = '\nh1, h2 { page-break-before: always; }\n'

    text_report.total_fixes = (
        text_report.double_spaces_fixed +
        text_report.ocr_ligatures_fixed +
        text_report.smart_quotes_normalized +
        text_report.encoding_issues_fixed +
        text_report.punctuation_fixed +
